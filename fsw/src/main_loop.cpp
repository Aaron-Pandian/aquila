...
int main() {
    ...
    // Read header line and build column map
    std::string line;
    ...
    const ColumnMap colmap = parse_header_map(line);
    if (!require_columns(colmap)) {
        std::cerr << "Input log missing required columns. Header was:\n" << line << std::endl;
        return 1;
    }
    ...
    while (std::getline(input_file, line)) {
        CsvRow row{};
        if (!parse_log_line(line, colmap, row)) {
